...
joins:
  - Holdout_HCP_Universe.abbott_customer_id → Gastro_prod_sales_260605.abv_customer_id (one-to-one, per relationships.yaml) — used ONLY to restrict to the in-scope HCP universe (RULE-102). Restricts to the 700-HCP holdout scope (RULE-102; material, the holdout is a strict subset of the 1,040-HCP universe). No demographics join (enrichment off, user choice).
filters_applied:
...
output_grain: full grain — one row per HCP (abbott_customer_id) across the holdout universe (700 HCPs), including doctors with no CD NBRx at all. 700 rows is intended (not a slide-size violation).
...
assumptions:
  ...
  - Universe = the 700-HCP holdout scope incl. zero-CD doctors; one row each. (user confirmed)
