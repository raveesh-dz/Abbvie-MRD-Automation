...
headline: 670 of 1,040 gastro HCPs (64%) are SKYRIZI Holdouts for Crohn's new prescribing — but 627 of those simply wrote no CD NBRx in the last 13 weeks; only 43 active prescribers are still defaulting to STELARA.
methodology: For each HCP in the holdout universe we summed SKYRIZI and STELARA Crohn's-Disease NBRx over the most recent 13 weeks (cur_13wk = frx1..frx13, week ending 2026-06-05 back to 2026-03-13). We divided STELARA NBRx by SKYRIZI NBRx; a ratio ≥ 0.5 (STELARA at least half of SKYRIZI) or any HCP with zero SKYRIZI is a Holdout (not yet moved to SKYRIZI); SKYRIZI-only or ratio < 0.5 is a Non-Holdout (SKYRIZI wins new prescribing). NBRx values are stored and selected by filter, never computed (RULE-005); all sums/ratios run in pandas.
filters_applied: data_type = NBRx; indication_code = CD (Crohn's); product_brand exactly SKYRIZI (OBI/SC maintenance, excludes SKYRIZI_IV) and exactly STELARA (excludes STELARA_IV, plain ustekinumab, and ustekinumab biosimilars); restricted to the 1,040-HCP holdout universe (RULE-102).
...
caveats: >
  ...
  - "Holdout" mixes two very different populations: 627 silent HCPs with NO CD NBRx at all
    (no recent new Crohn's starts to convert) and 43 active prescribers still writing STELARA.
    Conversion effort should separate these — the 43 active STELARA-leaners are the true switch
    targets; the 627 silent HCPs are an activation / opportunity-sizing question, not a switch.
